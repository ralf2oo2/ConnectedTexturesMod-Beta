...
public class UnbakedQuad implements Cloneable {
...
//    }
    ...
    public void setUVBounds(float[] bounds, float[] newBounds) {
        for (int vertexId = 0; vertexId < 4; vertexId++) {
            Vertex vertex = vertexes[vertexId];
            vertex.u = MathHelper.lerp(MathUtil.getLerpProgress(vertex.u, bounds[0], bounds[2]), newBounds[0], newBounds[2]);
            vertex.v = MathHelper.lerp(MathUtil.getLerpProgress(vertex.v, bounds[1], bounds[3]), newBounds[1], newBounds[3]);
        }
    }
