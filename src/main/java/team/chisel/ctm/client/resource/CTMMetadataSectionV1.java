...
//        return fromJson(jsonObject, () -> Identifier.of(""));
//    }
    ...
    public static CTMMetadataSection fromJson(JsonObject jsonObject, Function<String, Identifier> identifierProvider) throws JsonParseException {
        ...
        if (jsonObject.has("layer")) {
            ...
            if (layerElement.isJsonPrimitive() && layerElement.getAsJsonPrimitive().isString()) {
                try {
                    metadata.layer = BlockRenderLayer.valueOf(layerElement.getAsString().toUpperCase(Locale.ROOT));
                } catch (IllegalArgumentException e) {
    ...
    public String toString() {
        return "CTMMetadataSectionV1(type=" + getType() + ", layer=" + getLayer() + ", additionalTextures=" + Arrays.deepToString(getAdditionalTextures()) + ", proxy=" + getProxy() + ", extraData=" + getExtraData() + ")";
    }
}
