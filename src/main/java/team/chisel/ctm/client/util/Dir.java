...
public enum Dir {
        ...
        for (Direction normal : Direction.values()) {
            ...
            if (normal == NORMAL) {
                ...
                for (int i = 0; i < ret.length; i++) {
                    // TODO: I kinda just guessed getFrontYOffset was getOffsetY, if this causes issues I'll have to check it out
     ...
     */
    public boolean isConnected(CTMLogic ctm, BlockStateView world, BlockPos pos, Direction side) {
     ...
     * @return True if the block is connected in the given Dir, false otherwise.
     */
    public boolean isConnected(CTMLogic ctm, BlockStateView world, BlockPos pos, Direction side, BlockState state) {
    ...
    public BlockPos applyConnection(BlockPos pos, Direction side) {
        BlockPos offset = getOffset(side);
    ...
    private Direction rotate(Direction facing, Direction axisFacing) {
        ...
        Direction.AxisDirection axisDir = axisFacing.getDirection();

        if (axisDir == Direction.AxisDirection.POSITIVE) {
            return facing.rotateClockwise(axis);
        }
